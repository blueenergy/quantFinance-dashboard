pipeline {
  ...
  stages {
    ...
    stage('Deploy to VolcEngine') {
      ...
      steps {
        // Expected Jenkins credentials: an SSH private key with ID 'volc_ssh_key'.
        // Provide VOLC_USER and VOLC_HOST as Jenkins environment variables or global node env.
        sshagent(credentials: ['volc_ssh_key']) {
          sh '''
            ssh -o StrictHostKeyChecking=no ${VOLC_USER}@${VOLC_HOST} <<'EOF'
